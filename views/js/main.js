...
/*
    this function has been rewritten to just change the classes for randomPizzaContainers
    to small, medium or large
 */
var resizePizzas = function(value) {
    ...
    document.querySelector("#pizzaSize").innerHTML = size.capitalize();

    // Change the size of the pizzas
    x = document.querySelectorAll(".randomPizzaContainer");
    for (i = 0; i < x.length; i++) {
        x[i].className = "randomPizzaContainer " + size;
    }
...
/*
    The pizzaElementGenerator() has been removed. The elements are created by cloning pizza0.
 */
var pizzaDiv = document.getElementById("randomPizzas");
var pizzaElement = document.getElementById("pizza0");
var pizzaClone, i;
for (i = 2; i < 100; i++) {
    pizzaClone = pizzaElement.cloneNode(true);
    pizzaClone.id = "pizza" + i;
    pizzaClone.getElementsByTagName('h4')[0].innerHTML = randomName(); //pizza name
    pizzaClone.getElementsByTagName('ul')[0].innerHTML = makeRandomPizza(); // ingredients
    pizzaDiv.appendChild(pizzaClone);
}
...
function updatePositions() {
    ...
    /*
        Costly calculations (sin) were taken out of the for-loop.
        They run now 5 times instead of 50 times.
     */
    var scroll = document.body.scrollTop / 1250;

    // (i % 5) runs from 0 to 4
    var i;
    var phase = [];
    for (i = 0; i < 5; i++) {
    ...
    for (i = 0; i < items.length; i++) {
        ...
        /*
            style.top is not for speed but better organization, all styling happens in one place
         */
        items[i].style.top = items[i].basicTop + 'px';
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    elem.className = 'mover';   //to set width and height in additional.css
    elem.src = "images/pizza.png";
    for (var i = 0; i < 50; i++) {
        /*
            Number of movers reduced from 200 to 50, empirical guess. Important!
            Cloning might not be quicker than creation but I did it with the pizzas,
            at least I am now consistent.
         */
        var clone = elem.cloneNode(true);
        clone.basicLeft = (i % cols) * s;
        clone.basicTop = Math.floor(i / cols) * s;
        /*
            I originally tried to drag these attributes into css and to update the position there.
            But I failed and think I was a bit over the top with this approach.
         */
        document.querySelector("#movingPizzas1").appendChild(clone);
    }
    updatePositions();
});
